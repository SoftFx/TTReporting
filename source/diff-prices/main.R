...
source('task_Diff_PricesLPexec.R')
cfg_path   <- "./configDocker/config.yaml"  # read-only bind mount
state_path <- "./dataDocker/state.json"     # read-write bind mount
symbols_setup_path <- "./configDocker/SymbSlippage.csv"     # read-write bind mount
types_setup_path <- "./configDocker/types.csv"
commands_setup_path <- "./configDocker/commands.csv"
last_run_path <- "./dataDocker/last_run.txt"     # read-write bind mount
task_exec_log_path <- "./dataDocker/task_exec_log.csv"
task_exec_file <- "./dataDocker/lpexec.csv"
########################################################################
res <- NULL
...
res <- execute_task_diff_prices_lpexec(config = cfg, last_run_path, symbols_setup_path, types_setup_path, commands_setup_path, task_exec_log_path, task_exec_file)
res
